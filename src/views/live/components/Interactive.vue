<template>
	<div class="interactive-container">
		...
		<van-pull-refresh style="padding:1rem 0 1.2rem 0" v-model="refreshing" @refresh="onRefresh">
			<van-list
					v-model="loading"
					:finished="finished"
                    :offset="300"
	...
	methods: {
		...
		async onLoad () {
			...
			if (!this.flag) {
				...
				let body = await Live.getCommentPageParam({ liveId: '5615', appType: 'APP_TYPE_TEACHER' })
			...
			try {
				...
				setTimeout(() => {
					...
					if (body.list.totalPages < this.pageNo || body.list.length < 20) {
						this.finished = true
					}
					this.loading = false
				}, 500)
...
<style lang="scss" scoped>
  ...
  .commentList-box #commentList li .comment-right .cr-left .crl-mid{
	...
	/*.no-data{*/
		/*width:3.41rem;*/
		/*height:2.65rem;*/
		/*background:url(${ctx}/images/live/icon_sign_empty.png) no-repeat center center;*/
		/*background-size:100% 100%;*/
		/*margin:1.44rem auto .6rem auto;*/
	/*}*/
	.no-txt{
		font-size:30px;
		color:#333;
